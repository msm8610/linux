...
static bool bam_dmux_cmd_data(struct bam_dmux_skb_dma *skb_dma)
{
	 ...
	 *
	 * Typically the Ethernet mode is not implemented correctly in the modems,
	 * they only send an Ethernet header for DHCP replies but not for normal
	 * packets.
	 *
	 * Therefore this driver only supports Raw-IP mode, optionally with
	 * qmap layer on top.
	 */
	...
	case 0x40:
		netdev_err(netdev, "ip\n");
		skb->protocol = htons(ETH_P_IP);
		break;
	case 0x60:
		netdev_err(netdev, "ipv6\n");
